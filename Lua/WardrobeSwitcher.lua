...
local NET_APPLY_REQUEST = NET.V1_APPLY_REQUEST
local NET_CLEAR_REQUEST = NET.V1_CLEAR_REQUEST
local NET_FORGET_REQUEST = NET.V1_FORGET_REQUEST
local NET_LOOK_APPLY = NET.V1_LOOK_APPLY
...
local NET_V2_DIVING_COMMAND = NET.V2_DIVING_COMMAND
local NET_V2_DIVING_STATE = NET.V2_DIVING_PROFILE_STATE
local COMMAND_SAVE = Core.COMMAND.Save
local COMMAND_SAVE_KEEP = Core.COMMAND.SaveKeep
local COMMAND_APPLY = Core.COMMAND.Apply
...
local function tr(key, fallback)
    ...
    local ok, localized = pcall(function()
        if TextManager == nil or not TextManager.ContainsTag(tag) then return nil end
        return TextManager.Get(tag)
    end)
    if ok and localized ~= nil then return tostring(localized) end
...
local statusKeys = {
    ...
    ["Saved; server keep requested"] = "status.server_keep_requested",
    ["Saved and removed"] = "status.saved_removed",
    ["Saved; kept equipped"] = "status.saved_kept_equipped",
...
local slots = {
    ...
    { key = "HealthInterface", label = "Health", labelKey = "slot.health", slot = InvSlotType.HealthInterface, optional = true }
}
...
local sessionActiveCharacterIds = {}
local authoritativeServerRevision = 0
...
Helpers.pendingDivingProfilesByCharacterId = {}
...
function Helpers.overrideGeneSplicerAppearanceEnabled()
    local bridge = Helpers.ensureVisualOverride and Helpers.ensureVisualOverride() or nil
    if bridge == nil then return false end
    local ok, enabled = pcall(function() return bridge.GetOverrideGeneSplicerAppearance() end)
    return ok and enabled == true
end

function Helpers.shouldCaptureWardrobeSlot(entry)
    return entry ~= nil and
        (entry.optional ~= true or Helpers.overrideGeneSplicerAppearanceEnabled())
end
...
local pendingRoundStartProtocolLook = nil
...
local reducerState = Core.newClientState({ clientSessionId = clientSessionId })
...
-- Every event refreshes the facade's only authoritative state snapshot. Marking
-- the panel dirty here keeps asynchronous ACK/state changes out of the UI cache.
...
local function serverSupportsSaveWithoutUnequip()
    return protocolMode == "v3" and
        math.floor((tonumber(serverCapabilities) or 0) / CAPABILITY_SAVE_WITHOUT_UNEQUIP) % 2 == 1
end
...
local function currentDomainLook()
    return Core.copyLook(reducerState.look)
end

local function legacyLookFromDomain(look, metadataBySlot)
    if look == nil then return {} end
    local projected = Core.toLegacyLook(look) or {}
    metadataBySlot = metadataBySlot or legacyLookMetadata
    for _, entry in ipairs(slots) do
        local value = projected[entry.key]
        local metadata = metadataBySlot[entry.key]
...
local function currentLegacyLook()
    return legacyLookFromDomain(reducerState.look)
end

local function currentAttachmentVisibility()
    local look = reducerState.look
    return look ~= nil and
        (Core.validateAttachmentVisibility(look.attachmentVisibility, look.hideHair == true) or
            Core.attachmentVisibilityFromLegacy(look.hideHair == true)) or
        Core.attachmentVisibilityFromLegacy(false)
end
...
local function applyCharacterState(state)
    legacyLookMetadata = copyLookData(state ~= nil and state.legacyLookMetadata or nil)
    lastEquipmentSignature = state ~= nil and state.lastEquipmentSignature or nil
    slotResults = state ~= nil and state.slotResults or {}
    lastNetworkApplyDiagnostics = state ~= nil and state.lastNetworkApplyDiagnostics or {}
    dispatchReducer({
        type = "RestoreLook",
        look = state ~= nil and state.look or nil,
        active = state ~= nil and state.active == true,
        autoApply = state ~= nil and state.autoApply == true
    })
    if protocolMode == "v3" then
        dispatchReducer({ type = "RevisionObserved", revision = authoritativeServerRevision })
    end
end
...
local function saveCharacterState(character)
    ...
    if isSinglePlayerClient() and
        not Core.hasLook(state.look) and
        not state.active and
        not state.autoApply then
    ...
    characterStates[key] = {
        ...
        legacyLookMetadata = copyLookData(legacyLookMetadata),
...
local function loadCharacterState(character)
    ...
    if isSinglePlayerClient() and state == nil and type(loadSinglePlayerProfileState) == "function" then
        state = loadSinglePlayerProfileState(character)
    end
    applyCharacterState(state)
    return state ~= nil and Core.hasLook(state.look)
end
...
function Helpers.encodePersistentClientLook(
    ...
    if active == nil then active = state.active == true end
    if auto == nil then auto = state.autoApply == true end
    local visibility
    if type(visibilityValue) == "table" then
        visibility = Core.validateAttachmentVisibility(visibilityValue, false) or
            Core.attachmentVisibilityFromLegacy(false)
    elseif type(visibilityValue) == "boolean" then
        visibility = Core.attachmentVisibilityFromLegacy(visibilityValue)
    else
...
local function serverSupportsCrewDivingProfiles()
    return protocolMode == "v3" and
        math.floor((tonumber(serverCapabilities) or 0) /
            CAPABILITY_CREW_DIVING_PROFILES) % 2 == 1
end

function Helpers.shouldPreserveSinglePlayerCrewRender(character)
    if Helpers.userDataMember(character, "Removed") == true then return false end
    if isSinglePlayerClient() then return Helpers.singlePlayerCharacterEligible(character) end
    return Helpers.isMultiplayerClient() and serverSupportsCrewTargeting() and
...
function Helpers.singlePlayerProfileLineState(line)
    ...
    return {
        look = Core.copyLook(parsed.look),
        active = false,
        autoApply = parsed.autoApply == true or parsed.active == true,
        legacyLookMetadata = copyLookData(legacyLook),
...
function Helpers.restorePersistentClientLookLine(line, source)
    ...
    for part in tostring(line):gmatch("[^|]+") do
        ...
        elseif colorFields[name] ~= nil then
            ...
            if managedSlots[colorFields[name]] then restoredColors[colorFields[name]] = color end
    ...
    slotResults = {}
    for _, entry in ipairs(slots) do
        slotResults[entry.key] = restoredLook[entry.key] ~= nil and "Saved look needs to be applied again." or "Empty"
    end
    dispatchReducer({
        type = "RestoreLook",
        look = domainLook,
        active = false,
        autoApply = active == true or auto == true
    })
    reducerHasUnboundLook = true
    lastOperation = reducerState.autoApply and
        "Saved look will be reapplied in the next scene." or
...
persistClientLook = function(domainLook, viewModel)
    domainLook = domainLook or currentDomainLook()
    local lookData = currentLegacyLook()
    local captured = domainLook ~= nil and domainLook.captured == true
    ...
    if domainLook ~= nil then
        ...
        visibility = Core.validateAttachmentVisibility(
    ...
    if Helpers.isMultiplayerClient() and
        profileCharacter ~= Helpers.actualControlledCharacter() then
        return true
    end
...
clearPersistentClientLook = function()
    ...
    if Helpers.isMultiplayerClient() and
        controlled() ~= Helpers.actualControlledCharacter() then
        return true
    end
    local persistence = ensureWardrobePersistence()
    if persistence == nil then
...
function Helpers.parseDivingProfileLine(line)
    if line == nil or tostring(line) == "" then return nil end
    local mode = nil
    local captured = nil
    local look = {}
    local colors = {}
    local seen = {}
    local slotSet = {}
    for _, entry in ipairs(slots) do slotSet[entry.key] = true end

    for part in tostring(line):gmatch("[^|]+") do
        local name, value = part:match("^([^=]+)=(.*)$")
        if name == nil or seen[name] then return nil end
        seen[name] = true
        if name == "mode" then
            mode = tonumber(value)
            if mode == nil or mode % 1 ~= 0 or mode < Helpers.DIVING_MODE_NONE or
                mode > Helpers.DIVING_MODE_CUSTOM then return nil end
        elseif name == "captured" then
            if value == "true" then captured = true
            elseif value == "false" then captured = false
            else return nil end
        elseif name:sub(-#"Color") == "Color" and slotSet[name:sub(1, -#"Color" - 1)] then
            ...
            colors[key] = color
        elseif slotSet[name] then
            local identifier, displayName = value:match("^([^,]+),(.*)$")
            identifier = identifier ~= nil and Helpers.unescapePersistentValue(identifier) or nil
            if identifier == nil or identifier == "" or #identifier > 256 then return nil end
            look[name] = {
                identifier = identifier,
                itemId = 0,
                name = Helpers.unescapePersistentValue(displayName or ""),
    ...
    if mode == nil or captured == nil then return nil end
    for key, color in pairs(colors) do
        if look[key] == nil then return nil end
        look[key].color = color
    end
    return { mode = mode, captured = captured, look = look, loaded = true }
end

function Helpers.encodeDivingLook(profile)
    profile = profile or {}
    local look = profile.look or {}
    local parts = { "captured=" .. tostring(profile.captured == true) }
    for _, entry in ipairs(slots) do
        local value = look[entry.key]
        if value ~= nil and tostring(value.identifier or "") ~= "" then
            parts[#parts + 1] = entry.key .. "=" ..
                Helpers.escapePersistentValue(value.identifier) .. "," ..
                Helpers.escapePersistentValue(value.name or "")
            if value.color ~= nil then
                parts[#parts + 1] = entry.key .. "Color=" .. tostring(value.color)
            end
        end
    end
    return table.concat(parts, "|")
...
function Helpers.localDivingProfile(character)
    ...
    local profile = Helpers.divingProfilesByKey[key]
    if profile == nil then
        profile = {
            mode = Helpers.DIVING_MODE_NONE,
            captured = false,
            look = {},
            loaded = not persistable,
            nextLoadTick = 0
        }
...
function Helpers.persistDivingProfile(character, profile)
    local targetId = Helpers.multiplayerCrewTargetId(character)
    if targetId ~= nil then
        if not serverSupportsCrewDivingProfiles() or Networking == nil then
            return false, "server does not support persistent crew diving profiles"
        end
        local look = nil
        if profile.captured == true then
            local reason
            look, reason = Core.fromLegacyLook(
                profile.look,
                true,
                false,
                Core.attachmentVisibilityFromLegacy(false),
        ...
        local ok, reason = pcall(function()
            local message = Networking.Start(NET_V2_DIVING_COMMAND)
            local written, writeReason = Core.writeDivingProfile(message, {
                characterId = targetId,
                mode = tonumber(profile.mode) or Helpers.DIVING_MODE_NONE,
        ...
        return ok == true, ok and nil or tostring(reason)
    end
    local key, persistable = Helpers.divingProfileStorageKey(character)
    if not persistable then return true end
    local persistence = ensureWardrobePersistence()
    if persistence == nil then
        return false, persistenceFailureReason("C# wardrobe persistence is unavailable")
    end
    local ok, saved = pcall(function()
        return persistence.SaveDivingProfile(
            key,
            tonumber(profile.mode) or Helpers.DIVING_MODE_NONE,
            Helpers.encodeDivingLook(profile)
        )
    end)
    if ok and saved == true then return true end
    return false, ok and
        persistenceFailureReason("C# SaveDivingProfile returned false") or tostring(saved)
end

function Helpers.applyServerDivingProfile(profile)
    if type(profile) ~= "table" then return false end
    local characterId = tonumber(profile.characterId) or 0
    local character = Helpers.findEntityById(characterId)
    if character == nil then
        local pending = Helpers.pendingDivingProfilesByCharacterId[characterId]
        if pending == nil then
            local count = 0
            for _ in pairs(Helpers.pendingDivingProfilesByCharacterId) do count = count + 1 end
            if count >= 256 then return false end
        end
        profile.receivedTick = pending ~= nil and pending.receivedTick or globalTick
        Helpers.pendingDivingProfilesByCharacterId[characterId] = profile
        return false
    end
    local look = {}
    if profile.captured == true then
        local reason
        look, reason = Core.toLegacyLook(profile.look)
        if look == nil then
            Helpers.debugLog("Ignored invalid crew diving look: " .. tostring(reason))
            return false
        end
    end
    ...
    Helpers.divingProfilesByKey[key] = {
        mode = tonumber(profile.mode) or Helpers.DIVING_MODE_NONE,
        captured = profile.captured == true,
        look = look,
        loaded = true
    }
    Helpers.pendingDivingProfilesByCharacterId[characterId] = nil
...
function Helpers.sameRuntimeItem(left, right)
    ...
    return leftId > 0 and leftId == rightId
end

function Helpers.isIgnoredWardrobeItem(character, item)
    if character == nil or item == nil then return false end
    return not Helpers.overrideGeneSplicerAppearanceEnabled() and
        Helpers.sameRuntimeItem(item, getSlotItem(character, InvSlotType.HealthInterface))
end

function Helpers.hasHighPressureAffliction(character)
    local override = Helpers.ensureVisualOverride()
    if override ~= nil then
        local ok, active = pcall(function()
            return override.HasHighPressureAffliction(character)
        end)
        if ok then return active == true end
    end
    local ok, active = pcall(function()
        return character.InPressure
    end)
    return ok and active == true
end

function Helpers.isDivingSuitItem(item)
    if item == nil then return false end
    local override = Helpers.ensureVisualOverride()
    if override ~= nil then
        local ok, divingSuit = pcall(function()
            return override.IsDivingSuitItem(item)
        end)
        if ok then return divingSuit == true end
    end
    local ok, divingSuit = pcall(function()
        return item.HasTag("deepdiving") or item.HasTag("deepdivinglarge")
    end)
    return ok and divingSuit == true
end

function Helpers.isLockedWardrobeItem(item)
    if item == nil then return false end
...
function Helpers.hasSavedLook()
    return Core.hasLook(reducerState.look)
end
...
function Helpers.deactivateCachedCharacterStates(character, preserveAutoApply)
    ...
    if state ~= nil then
        state.active = false
        if preserveAutoApply ~= true then state.autoApply = false end
...
function Helpers.preserveSceneTransitionLookIntent()
    local shouldReapplyCurrentLook = Helpers.hasSavedLook() and
        (reducerState.active or reducerState.autoApply)
    dispatchReducer({ type = "PrepareSceneTransition", reapply = shouldReapplyCurrentLook })
    lastEquipmentSignature = nil
    lastServerAutoApplySignature = nil

    for _, state in pairs(characterStates) do
        if Helpers.stateHasSavedLook(state) and (state.active == true or state.autoApply == true) then
            state.active = false
...
function Helpers.savedLookSummary(lookData, captured)
    lookData = lookData or currentLegacyLook()
    if captured == nil then captured = reducerState.look ~= nil and reducerState.look.captured == true end
...
function Helpers.managedWearableSlotsAreEmpty(character)
    ...
    for _, entry in ipairs(slots) do
        local item = Helpers.shouldCaptureWardrobeSlot(entry) and getSlotItem(character, entry.slot) or nil
        if item ~= nil and not Helpers.isIgnoredWardrobeItem(character, item) then
            return false
        end
    end
    return true
...
unequipItem = function(character, item)
    ...
    if Helpers.isLockedWardrobeItem(item) then
        pcall(function()
            item.Drop(character)
        end)
        return isClear()
...
function Helpers.snapshot(character)
    ...
    for _, entry in ipairs(slots) do
        ...
        if not Helpers.shouldCaptureWardrobeSlot(entry) or Helpers.isIgnoredWardrobeItem(character, item) then
            data[entry.key] = nil
        else
            data[entry.key] = item
        end
...
function Helpers.setFashionSlotMask(character, lookData, diving)
    ...
    lookData = lookData or currentLegacyLook()
    for _, entry in ipairs(slots) do
        if entry.optional == true and lookData[entry.key] == nil then
            -- An omitted optional slot is outside this saved look.
        elseif lookData[entry.key] ~= nil then
...
function Helpers.setAttachmentVisibilityVisual(character, value, diving)
    ...
    if type(value) == "table" then
        visibility = Core.validateAttachmentVisibility(value, false) or
            Core.attachmentVisibilityFromLegacy(false)
    elseif type(value) == "boolean" then
        visibility = Core.attachmentVisibilityFromLegacy(value)
    else
        visibility = currentAttachmentVisibility()
    end
    local forceHide, forceShow, maskReason = Core.attachmentVisibilityMasks(visibility)
...
function Helpers.huskVisualsHidden()
    local bridge = Helpers.ensureVisualOverride()
    if bridge == nil then return false end
    local ok, hidden = pcall(function() return bridge.GetHideHuskVisuals() end)
    return ok and hidden == true
end

function Helpers.setHuskVisualsHidden(hidden)
    local bridge = Helpers.ensureVisualOverride()
    if bridge == nil then return false end
    local ok, result = pcall(function()
        return bridge.SetHideHuskVisuals(hidden == true)
    end)
    if not ok then
        Helpers.log("Husk-appearance update failed: " .. tostring(result) ..
            ". Reload the mod so LuaCs recompiles the C# plugin.")
        return false
    end
    return result == true
...
function Helpers.unequipOnSaveEnabled()
    local bridge = Helpers.ensureVisualOverride()
    if bridge == nil then return true end
    local ok, enabled = pcall(function() return bridge.GetUnequipOnSave() end)
    return not ok or enabled == true
end

function Helpers.setUnequipOnSaveEnabled(enabled)
    local bridge = Helpers.ensureVisualOverride()
    if bridge == nil then return false end
    local ok, result = pcall(function()
        return bridge.SetUnequipOnSave(enabled == true)
    end)
    if not ok then
...
function Helpers.setOverrideGeneSplicerAppearanceEnabled(enabled)
    local bridge = Helpers.ensureVisualOverride()
    if bridge == nil then return false end
    local ok, result = pcall(function()
        return bridge.SetOverrideGeneSplicerAppearance(enabled == true)
    end)
    if not ok then
        Helpers.log("Gene-splicer appearance update failed: " .. tostring(result) ..
            ". Reload the mod so LuaCs recompiles the C# plugin.")
        return false
    end
    return result == true
    ...
    local ok, reason = pcall(function()
        ...
        if Core.isDivingCommand(command.kind) or
            ...
            local written, writeReason = writer(message, {
                ...
                includeHealthInterface = command.includeHealthInterface
            })
            if not written then error(writeReason) end
        else
...
function Helpers.sendNextProtocolCommand()
    ...
    if command.kind == COMMAND_SAVE_KEEP and not serverSupportsSaveWithoutUnequip() then
        table.remove(protocolCommandQueue, 1)
        if command.reducerOwned == true then
            dispatchReducer({
                type = "CommandSendFailed",
                operationId = command.operationId,
                reason = "server does not support saving without unequipping"
            })
        end
        Helpers.sendNextProtocolCommand()
        return false
    end
    local baseRevision = authoritativeServerRevision
...
function Helpers.sendV2Hello(forceSnapshot)
    ...
    local ok, reason = pcall(function()
        ...
        local written, writeReason = Core.writeClientHello(message, clientSessionId, 0x7F)
        if not written then error(writeReason) end
        Networking.Send(message)
    end)
    if ok then
...
function Helpers.selectV1Protocol(reason)
    ...
    sessionActiveCharacterIds = {}
    authoritativeServerRevision = 0
...
function Helpers.selectV2Protocol(revision, capabilities)
    ...
    authoritativeServerRevision = math.max(authoritativeServerRevision, serverRevision)
...
function Helpers.flushPendingVisibilitySync()
    ...
    dispatchReducer({
        ...
        attachmentVisibility = currentAttachmentVisibility(),
        remote = true,
...
function Helpers.queueProtocolCommand(kind, lookData, captured, operationId, reducerOwned, domainLookOverride, targetCharacterId)
    if not Helpers.isMultiplayerClient() or Networking == nil then return false end
    if kind == COMMAND_SAVE_KEEP and protocolMode ~= "probing" and
        not serverSupportsSaveWithoutUnequip() then
        Helpers.debugLog("The server does not support saving without unequipping.")
        return false
    end
    ...
    elseif kind == COMMAND_SAVE or kind == COMMAND_SAVE_KEEP or kind == COMMAND_APPLY then
        local visibility = currentAttachmentVisibility()
...
function Helpers.divingOverrideActive(character)
    local runtime = character ~= nil and Helpers.divingRuntime(character) or nil
    return runtime ~= nil and runtime.active == true
end
...
function Helpers.currentDivingSuitLook(character)
    ...
    for _, entry in ipairs(slots) do
        ...
        if Helpers.isDivingSuitItem(item) then
            found = true
            look[entry.key] = {
                identifier = Helpers.itemIdentifier(item),
                itemId = Helpers.itemEntityId(item),
...
function Helpers.updateDivingAppearances(force)
    if not force and globalTick < Helpers.nextDivingPollTick then return end
...
function Helpers.forgetOwnedCharacter(key, character)
    ...
    Helpers.pendingDivingProfilesByCharacterId[id], sessionActiveCharacterIds[id] = nil, nil
...
function Helpers.queueDivingCommand(character, kind, profile)
    ...
    protocolCommandQueue[#protocolCommandQueue + 1] = {
        ...
        includeHealthInterface = Helpers.overrideGeneSplicerAppearanceEnabled(),
...
function Helpers.processDivingTransport()
    for id, profile in pairs(Helpers.pendingDivingProfilesByCharacterId) do
        if globalTick - profile.receivedTick > PendingNetworkMessageMaxTicks then
            Helpers.pendingDivingProfilesByCharacterId[id] = nil
        end
    end
...
function Helpers.divingModeButtonLabel(mode)
    if mode == Helpers.DIVING_MODE_SUIT then return tr("button.diving_mode_suit") end
    if mode == Helpers.DIVING_MODE_CUSTOM then return tr("button.diving_mode_custom") end
    return tr("button.diving_mode_none")
end
...
clientEffectAdapters.Capture = function(currentEffect)
    ...
    if not context.remote then
        ...
        if not Helpers.setFashionSlotMask(character, lookData) or
            not Helpers.setAttachmentVisibilityVisual(character, visibility) then
            Helpers.abortFashionTransaction(character)
...
clientEffectAdapters.Unequip = function()
    ...
    local shouldUnequip = Helpers.unequipOnSaveEnabled()
    local processedItems = {}
    ...
    for _, entry in ipairs(slots) do
        ...
        elseif not shouldUnequip then
            processedItems[item] = true
            results[entry.key] = "Saved; kept equipped"
        else
            processedItems[item] = true
    ...
    elseif not shouldUnequip then
        message = "Saved current outfit: " .. tostring(context.capturedSprites) ..
            " wearable sprites captured; equipment kept."
    else
        message = "Saved current outfit: " .. tostring(context.capturedSprites) ..
...
clientEffectAdapters.SendCommand = function(currentEffect)
    ...
    local commandKind = currentEffect.kind == COMMAND_SAVE and
        not Helpers.unequipOnSaveEnabled() and COMMAND_SAVE_KEEP or currentEffect.kind
    ...
    if currentEffect.kind == COMMAND_SAVE and pendingSaveContext ~= nil then
        ...
        local results = {}
        for _, entry in ipairs(slots) do
            results[entry.key] = context.lookData[entry.key] ~= nil and
                (commandKind == COMMAND_SAVE_KEEP and "Saved; server keep requested" or
                    "Saved; server removal requested") or "Empty"
        end
        slotResults = results
        lastCharacter = context.character
        lastEquipmentSignature = nil
        lastServerAutoApplySignature = nil
        Helpers.log(commandKind == COMMAND_SAVE_KEEP and
            "Saved current outfit; server was asked to keep equipment worn." or
            "Saved current outfit; server-side removal requested for multiplayer.")
        pendingSaveContext = nil
    end
    return {
...
function Helpers.handleNoControlledCharacter()
    ...
    if Helpers.hasSavedLook() then
        ...
        if next(slotResults) == nil then
            local lookData = currentLegacyLook()
            for _, entry in ipairs(slots) do
                slotResults[entry.key] = lookData[entry.key] ~= nil and "Saved look needs to be applied again." or "Empty"
            end
        end
    else
        slotResults = {}
...
function Helpers.handleControlledCharacterChange(character)
    ...
    if lastCharacter == nil and reducerHasUnboundLook and Core.hasLook(reducerState.look) then
        bootstrapState = {
            look = Core.copyLook(reducerState.look),
            active = reducerState.active == true,
            autoApply = reducerState.autoApply == true,
            legacyLookMetadata = copyLookData(legacyLookMetadata),
            lastEquipmentSignature = lastEquipmentSignature,
            slotResults = slotResults,
            lastNetworkApplyDiagnostics = lastNetworkApplyDiagnostics
    ...
    if bootstrapState ~= nil then
        applyCharacterState(bootstrapState)
        reducerHasUnboundLook = false
        hadState = true
    else
        hadState = loadCharacterState(character)
    end
    ...
    if isSinglePlayerClient() and not hadState and
        ...
        (sourceState.active == true or sourceState.autoApply == true) then
        ...
        if targetKey ~= nil and targetProfileKey ~= nil and
            ...
            local transferredState = {
                look = Core.copyLook(sourceState.look),
                active = false,
                autoApply = true,
                legacyLookMetadata = copyLookData(sourceState.legacyLookMetadata),
                lastEquipmentSignature = nil,
                slotResults = {},
                lastNetworkApplyDiagnostics = {},
            ...
            local transferredLook = Core.toLegacyLook(transferredState.look) or {}
            for _, entry in ipairs(slots) do
                transferredState.slotResults[entry.key] =
                    transferredLook[entry.key] ~= nil and
                    "Saved look needs to be applied again." or "Empty"
            end
...
function Helpers.queueSinglePlayerProfileRestore(character)
    ...
    if character == controlled() then
        if character ~= lastCharacter then return true end
        characterStates[runtimeKey] = nil
        applyCharacterState(state)
        return true
    end
    if state.autoApply ~= true then return false end
...
function Helpers.belongsToLocalWardrobeState(characterId)
    ...
    if characterId == Helpers.characterEntityId(controlled()) then return true end
                ...
                if applied then
                    ...
                    for _, entry in ipairs(slots) do
                        state.slotResults[entry.key] =
                            lookData[entry.key] ~= nil and "Saved and applied" or "Empty"
                    end
                    pendingSinglePlayerRestores[runtimeKey] = nil
                    Helpers.debugLog(
...
function Helpers.applyPendingRoundStartNetworkLook(character)
    ...
    local protocolLook = Core.copyLook(pendingRoundStartProtocolLook)
    pendingRoundStartNetworkLook = nil
    pendingRoundStartNetworkCharacterKey = nil
    pendingRoundStartNetworkRevision = nil
    pendingRoundStartHideHair = false
    ...
    return Helpers.handleNetworkLookApply(
        Helpers.characterEntityId(character),
        networkLook,
        protocolRevision,
        hairHidden,
        protocolLook
...
function Helpers.handleNetworkLookApply(
    ...
    if protocolRevision ~= nil and Helpers.belongsToLocalWardrobeState(characterId) then
        ...
        if effectsContain(effects, "IgnoredDuplicateState") then
            ...
            sessionActiveCharacterIds[characterId] = true
...
function Helpers.handleNetworkLookClear(characterId, protocolRevision, protocolLook, retryState)
    ...
    local function finishClear()
        ...
        sessionActiveCharacterIds[tonumber(characterId)] = nil
...
if Networking ~= nil then
    ...
    Networking.Receive(NET_V2_HELLO, function(message)
        ...
        Networking.Receive(NET_V2_ACK, function(message)
            ...
            authoritativeServerRevision = math.max(
                authoritativeServerRevision,
                tonumber(ack.revision) or 0
            )
        ...
        Networking.Receive(NET_V2_STATE, function(message)
            ...
            if belongsToControlledCharacter then
                authoritativeServerRevision = math.max(
                    authoritativeServerRevision,
            ...
            if belongsToControlledCharacter and
                ...
                state.look ~= nil then
                local localLook = currentDomainLook()
                if localLook ~= nil then
                    state.look.attachmentVisibility = Core.validateAttachmentVisibility(
        ...
        Networking.Receive(NET_V2_DIVING_STATE, function(message)
            if Helpers.supportsDivingSync() then return end
            local profile, reason = Core.tryReadDivingProfile(message)
            if profile == nil then
                Helpers.debugLog("Ignored malformed crew diving profile: " .. tostring(reason))
                return
            end
            if protocolMode ~= "v3" then Helpers.selectV2Protocol(0) end
            Helpers.applyServerDivingProfile(profile)
        end)
end
...
function Helpers.dumpDebugLog()
    ...
    emit("savedLookCaptured=" .. tostring(reducerState.look ~= nil and reducerState.look.captured == true) ..
        ", activeLook=" .. tostring(reducerState.active) ..
        ", autoApplyLook=" .. tostring(reducerState.autoApply))
    ...
    local projectedLook = currentLegacyLook()
    for _, entry in ipairs(slots) do
        local current = character ~= nil and getSlotItem(character, entry.slot) or nil
        local saved = projectedLook[entry.key]
        emit(
            entry.key ..
...
function Helpers.clientViewModelSnapshot(character, overrideState)
    ...
    local viewCaptured = reducerView.look ~= nil and reducerView.look.captured == true
    local viewHideHair = reducerView.look ~= nil and reducerView.look.hideHair == true
    local viewAttachmentVisibility = reducerView.look ~= nil and
        (Core.validateAttachmentVisibility(reducerView.look.attachmentVisibility, reducerView.look.hideHair == true) or
            Core.attachmentVisibilityFromLegacy(reducerView.look.hideHair == true)) or
        Core.attachmentVisibilityFromLegacy(false)
    local divingProfile = Helpers.divingProfile(character)
    return {
        ...
        divingSessionOnly = divingProfile.sessionOnly == true or
            (Helpers.isMultiplayerClient() and character ~= Helpers.actualControlledCharacter() and
                not serverSupportsCrewDivingProfiles()),
        divingLocalOnly = Helpers.isMultiplayerClient() and not Helpers.supportsDivingSync() and
            not (character ~= Helpers.actualControlledCharacter() and serverSupportsCrewDivingProfiles()),
...
buildWindow = function()
    ...
    if not advancedPanelOpen then
        ...
        if view.divingMode == Helpers.DIVING_MODE_CUSTOM then
            Helpers.addButtonPair(
                list,
                tr("button.save_diving_outfit"),
                function() Helpers.saveCustomDivingLook() end,
                overrideState.ready and not view.busy,
                Helpers.divingModeButtonLabel(view.divingMode),
                function() Helpers.cycleDivingMode() end,
                not view.busy
            )
        else
            Helpers.addButton(
        ...
        Helpers.addButton(
            list,
            view.hideHuskVisuals and
                tr("button.hide_husk_yes") or
                tr("button.hide_husk_no"),
            function()
                if not Helpers.setHuskVisualsHidden(not view.hideHuskVisuals) then
                    Helpers.log("Husk appearance could not be updated.")
                end
            end,
            true,
            overrideState.ready
        ...
        Helpers.addButton(
            list,
            view.unequipOnSave and
                tr("button.unequip_on_save_yes") or
                tr("button.unequip_on_save_no"),
            function()
                if not Helpers.setUnequipOnSaveEnabled(not view.unequipOnSave) then
                    Helpers.log("Unequip-on-save setting could not be saved.")
                end
            end,
            true,
            overrideState.ready and not view.busy
        ...
        Helpers.addButton(
            ...
            view.overrideGeneSplicerAppearance and
                tr("button.override_gene_splicer_yes") or
                tr("button.override_gene_splicer_no"),
            function()
                if not Helpers.setOverrideGeneSplicerAppearanceEnabled(
                    not view.overrideGeneSplicerAppearance) then
                    Helpers.log("Gene-splicer appearance setting could not be saved.")
                end
            end,
            true,
            overrideState.ready and not view.busy
...
buildAttachmentVisibilityWindow = function()
        ...
        Helpers.addButton(
            ...
            function()
                local nextVisibility =
                    Core.validateAttachmentVisibility(view.attachmentVisibility, false) or
                    Core.attachmentVisibilityFromLegacy(false)
    ...
    Helpers.addButton(list, tr("button.all_auto"), function()
        Helpers.updateAttachmentVisibility(Core.attachmentVisibilityFromLegacy(false))
    end, true, view.canSetAttachmentVisibility)
...
Hook.Add("roundStart", "barowardrobeswitcher.notice", function()
    ...
    if Helpers.hasSavedLook() and reducerState.autoApply then
...
Hook.Add("character.created", "barowardrobeswitcher.profile-character-created", function(character)
    ...
    local pendingDiving = Helpers.pendingDivingProfilesByCharacterId[characterId]
    if pendingDiving ~= nil then Helpers.applyServerDivingProfile(pendingDiving) end
    if not isSinglePlayerClient() then return end
    local attempts = 0
...
Hook.Add("roundEnd", "barowardrobeswitcher.cleanup", function()
    ...
    local selectedId = Helpers.characterEntityId(controlled())
    local crewTargetWasActive = Helpers.isMultiplayerClient() and
        selectedId > 0 and tonumber(ownerId) ~= nil and
        selectedId ~= tonumber(ownerId) and sessionActiveCharacterIds[selectedId] == true
