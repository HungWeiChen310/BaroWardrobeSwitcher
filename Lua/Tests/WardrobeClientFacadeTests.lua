...
hooks.think()
assert(removedWidgets == removesBeforeGeneSplicerOverride and liveOverlayRoots == 1 and
...
hooks.think()
assert(removedWidgets == removesBeforeDisableUnequip and liveOverlayRoots == 1 and
...
hooks.think()
assert(removedWidgets == removesBeforeHideHusk and liveOverlayRoots == 1 and
...
do
    ...
    hooks.think()
    assert((targetDropdown.SelectedData == multiplayerBot) and
...
do
    ...
    assert(WardrobeCore.writeServerHello(hello, 0, 127))
    ...
    local legacyBot = makeCharacter(980, 980, "Legacy crew", true)
    legacyBot.InPressure = true
    Character.CharacterList[#Character.CharacterList + 1] = legacyBot
    local function legacyProfile(id)
        local packet = newNetworkBuffer(WardrobeCore.NET.V2_DIVING_PROFILE_STATE)
        assert(WardrobeCore.writeDivingProfile(packet, { characterId = id, mode = 2, captured = true, look = custom }))
        networkHandlers[WardrobeCore.NET.V2_DIVING_PROFILE_STATE](packet.FinalizeForTransport())
    end
    legacyProfile(980)
    assert(H.divingProfile(legacyBot).mode == 0, "new protocol accepted the conflicting legacy codec")
    H.selectV2Protocol(4, 63)
    legacyProfile(980)
    assert(H.divingProfile(legacyBot).mode == 2 and divingActive[980], "old server crew diving compatibility was lost")
    legacyProfile(981)
    assert(H.pendingDivingProfilesByCharacterId[981] ~= nil)
    for _ = 1, 1802 do hooks.think() end
    assert(H.pendingDivingProfilesByCharacterId[981] == nil, "legacy entity wait was not bounded")
    hooks.stop()
