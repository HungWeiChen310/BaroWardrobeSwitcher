...
- On a current multiplayer server, confirm `Wardrobe target` cycles from the local player through friendly living human bots only. Save/Apply target only the selected character, the selector is disabled while a command is pending, and each player or bot keeps an independent look. On an older server without the capability, the selector remains self-only.
