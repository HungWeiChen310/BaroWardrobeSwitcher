...
- `Lua/WardrobeSwitcher.lua` is the client adapter. It owns the configurable-key two-page UI, capability-gated bot target selection, character/inventory hooks, persistence calls, renderer calls, and v1/protocol-5 network negotiation.
...
Multiplayer connection, round, and LuaCs `character.created` events continue to rebind active sessions to new Character entity IDs. A bounded event-triggered retry handles the short assignment race during respawn. An active controlled character computes a six-slot equipment signature every six ticks, with immediate invalidation on equipment events; a signature change refreshes the existing local renderer session without capture, persistence, or a wardrobe network command. There is no per-frame full-client or full-inventory scan.
...
- Diving: `DivingProfiles.json`, schema 1, independent local cosmetic profiles.
...
Wire look schema 4 and persistence schema 5 are separate constants. Each network slot carries its identifier, a color-presence bit, and an optional packed color; the optional marker/version/mask tail remains independent of persistence.
...
## Diving and input lifetimes

Diving profiles are independent of the normal reducer look. Their temporary render payload is never eligible for normal-session reuse. A base look change invalidates the diving render signature; pressure exit restores the latest base intent. Failed activation cleans up and waits 60 ticks before retrying, while explicit actions can retry immediately. NPC diving profiles enter the bounded queue during the initial crew scan; loaded disabled profiles leave it. Character removal, replacement and round/session transitions release temporary state.

Stable single-player diving storage keys are cached by Character instance, checked against fingerprint ambiguity and discarded at crew scans, removal and session/round boundaries. Custom look signatures are cached until the user saves a new diving outfit. The steady-state workload must perform no capture, persistence or wardrobe network send.

UI callbacks freeze the character shown when each button was built, and reject stale-target clicks before dispatch. Keyboard focus is checked only after a shortcut hit. Status text wraps inside the scrollable panel; rendering at narrow sizes still requires in-game visual verification.
