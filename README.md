...
- A successful C# load prints:
  - `[Baro Wardrobe Switcher] C# visual override v0.3.6 initializing.`
...
- Multiplayer uses a small server-side Lua sync helper. The server stores saved wardrobe item IDs, performs the server-authoritative removal, and broadcasts apply/clear events so other clients with LuaCs and C# scripting enabled can see the active look.
